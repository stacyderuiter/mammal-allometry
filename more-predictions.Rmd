...
```{r, do-boot-hr}
if (!file.exists('fitted-models/hr_boot_22.RDS')){
...
saveRDS(hr_boot, 'fitted-models/hr_boot_22.RDS')
}else{
  hr_boot <- readRDS('fitted-models/hr_boot_22.RDS')
}
```

```{r, do-boot-sv}
if (!file.exists('fitted-models/sv_boot_22.RDS')){
sv_boot <- bootMer(sv_mod, 
                   FUN = predict_sv,
                   nsim = n_boot,
                   re.form = NULL,
                   type = 'parametric')
saveRDS(sv_boot, 'fitted-models/sv_boot_22.RDS')
}else{
  sv_boot <- readRDS('fitted-models/sv_boot_22.RDS')
}
```
...
```{r, cardiac-vs-mass-log-scale, fig.cap = 'Predicted cardiac output as a function of mass and habitat. Colored dots with error bars are mean values and percentile-based 95 percent confidence intervals from parametric bootstrap. Black solid line is based on Stahl (1967).'}
cardiac_mass_log <- gf_point(boot_median_cardiac ~ mean_mass, data = cardiac_preds,
  ...
  gf_line(187*mean_mass^0.81 ~ mean_mass, data = cardiac_preds,
          colour = 'black')
...
```{r, cardiac-mass-log-image, fig.show = 'hide', dev = 'jpeg', fig.cap = 'Predicted cardiac output as a function of mass and habitat. Colored dots with error bars are mean values and percentile-based 95 percent confidence intervals from parametric bootstrap. Black solid line is based on Stahl (1967).'}
cardiac_mass_log
```
...
```{r, do-boot-fr}
if (!file.exists('fitted-models/fr_boot_22.RDS')){
...
saveRDS(fr_boot, 'fitted-models/fr_boot_22.RDS')
}else{
  fr_boot <- readRDS('fitted-models/fr_boot_22.RDS')
}
```

```{r, do-boot-vt}
if (!file.exists('fitted-models/vt_boot_22.RDS')){
vt_boot <- bootMer(vt_mod, 
                   FUN = predict_vt,
                   nsim = n_boot,
                   re.form = NULL,
                   type = 'parametric')
saveRDS(vt_boot, 'fitted-models/vt_boot_22.RDS')
}else{
  vt_boot <- readRDS('fitted-models/vt_boot_22.RDS')
}
```
...
```{r, vent-vs-mass-log-scale, fig.cap = 'Predicted ventilation as a function of mass and habitat. Colored dots with error bars are mean values and percentile-based 95 percent confidence intervals from parametric bootstrap. Black solid line is based on Stahl (1967).'}
...
```{r, vent-mass-log-image, fig.show = 'hide', dev = 'jpeg', fig.cap = 'Predicted ventilation as a function of mass and habitat. Colored dots with error bars are mean values and percentile-based 95 percent confidence intervals from parametric bootstrap. Black solid line is based on Stahl (1967).'}
vent_mass_log
```
