...
```{r, do-boot-hr-diff}
if (!file.exists('fitted-models/hr_habitat_boot_22.RDS')){
...
saveRDS(hr_boot, 'fitted-models/hr_habitat_boot_22.RDS')
}else{
  hr_boot <- readRDS('fitted-models/hr_habitat_boot_22.RDS')
}
```

```{r, do-boot-sv-diff}
if (!file.exists('fitted-models/sv_habitat_boot_22.RDS')){
sv_boot <- bootMer(sv_mod, 
                   FUN = predict_sv,
                   nsim = n_boot,
                   re.form = NA,
                   type = 'parametric')
saveRDS(sv_boot, 'fitted-models/sv_habitat_boot_22.RDS')
}else{
...
```{r, do-boot-fr-diff}
if (!file.exists('fitted-models/fr_boot-diff_22.RDS')){
...
saveRDS(fr_boot, 'fitted-models/fr_boot-diff_22.RDS')
}else{
  fr_boot <- readRDS('fitted-models/fr_boot-diff_22.RDS')
}
```

```{r, do-boot-vt-diff}
if (!file.exists('fitted-models/vt_boot-diff_22.RDS')){
vt_boot <- bootMer(vt_mod, 
                   FUN = predict_vt,
                   nsim = n_boot,
                   re.form = NULL,
                   type = 'parametric')
saveRDS(vt_boot, 'fitted-models/vt_boot-diff_22.RDS')
}else{
  vt_boot <- readRDS('fitted-models/vt_boot-diff_22.RDS')
}
```
