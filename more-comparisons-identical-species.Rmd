...
### Note this is not needed b/c we make the predictions here with re_form NA, that is, excluding random effect variance; population-level predictions.
...
joint_species <- inner_join(mammal_hr %>% select(order,
                                      genus,
                                      species),
                            mammal_sv %>% select(order,
                                                 genus,
                                                 species))
...
cardiac_hr <- cardiac_hr %>%
  mutate(log10.body.mass = log10(mass.kg),
         habitat = c(rep('aquatic', 50),
                     rep('land', 50)),
         which_animal = sample(c(1:nrow(joint_species)),
                               size = 100, 
                               replace = TRUE),
         order = joint_species$order[which_animal],
         genus = joint_species$genus[which_animal],
         species = joint_species$species[which_animal],
         log10fr = 0) %>%
  dplyr::select(numb, which_animal, order, genus, species, log10.body.mass, log10fr, habitat)

cardiac_sv <- cardiac_sv %>%
  mutate(log10.body.mass = log10(mass.kg),
         habitat = c(rep('aquatic', 50),
                     rep('land', 50)),
         which_animal = cardiac_hr$which_animal,
         order = joint_species$order[which_animal],
         genus = joint_species$genus[which_animal],
         species = joint_species$species[which_animal],
         log10sv = 0) %>%
...
```{r, do-boot-hr}
if (!file.exists('fitted-models/hr_habitat_boot_identical-species.RDS')){
...
saveRDS(hr_boot, 'fitted-models/hr_habitat_boot_identical-species.RDS')
}else{
  hr_boot <- readRDS('fitted-models/hr_habitat_boot_identical-species.RDS')
}
```

```{r, do-boot-sv}
if (!file.exists('fitted-models/sv_habitat_boot_identical-species.RDS')){
sv_boot <- bootMer(sv_mod, 
                   FUN = predict_sv,
                   nsim = n_boot,
                   re.form = NA,
                   type = 'parametric')
saveRDS(sv_boot, 'fitted-models/sv_habitat_boot_identical-species.RDS')
}else{
  sv_boot <- readRDS('fitted-models/sv_habitat_boot_identical-species.RDS')
}
```
...
For each row of the dataset, make a set of `r n_boot` model predictions of ventilation. These predictions will include uncertainty in the model parameters (intercept and slopes). (Residual variation and order/genus/species variation is  not included). The mass estimates are different in the different datasets; here we use the mass estimate from the dataset to which the model (`fr` or `vt`) was originally fitted.
