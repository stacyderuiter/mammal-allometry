...
cardiac_hr <- cardiac_sv <- data.frame(numb = c(1:100),
                                       mass_kg = c(seq(from = 0.1, to = 5000, by = 100),
...
cardiac_hr <- cardiac_hr %>%
  mutate(log10_mass = log10(mass_kg),
         ...
         log10_fr = 0) %>%
  dplyr::select(numb, animal, order, genus, species, log10_mass, log10_fr, habitat)
...
cardiac_sv <- cardiac_sv %>%
  mutate(log10_mass = log10(mass_kg),
         ...
         log10_sv = 0) %>%
  dplyr::select(numb, log10_sv, log10_mass, habitat, order, genus, species) 
```
...
```{r, do-boot-hr-diff}
if (!file.exists('fitted-models/hr_habitat_boot_2022.RDS')){
...
saveRDS(hr_boot, 'fitted-models/hr_habitat_boot_2022.RDS')
}else{
  hr_boot <- readRDS('fitted-models/hr_habitat_boot_2022.RDS')
}
```

```{r, do-boot-sv-diff}
if (!file.exists('fitted-models/sv_habitat_boot_2022.RDS')){
sv_boot <- bootMer(sv_mod, 
                   FUN = predict_sv,
                   nsim = n_boot,
                   re.form = NA,
                   type = 'parametric')
saveRDS(sv_boot, 'fitted-models/sv_habitat_boot_2022.RDS')
}else{
  sv_boot <- readRDS('fitted-models/sv_habitat_boot_2022.RDS')
}
```
...
```{r, do-boot-fr-diff}
if (!file.exists('fitted-models/fr_boot-diff_2022.RDS')){
...
saveRDS(fr_boot, 'fitted-models/fr_boot-diff_2022.RDS')
}else{
  fr_boot <- readRDS('fitted-models/fr_boot-diff_2022.RDS')
}
```

```{r, do-boot-vt-diff}
if (!file.exists('fitted-models/vt_boot-diff_2022.RDS')){
vt_boot <- bootMer(vt_mod, 
                   FUN = predict_vt,
                   nsim = n_boot,
                   re.form = NULL,
                   type = 'parametric')
saveRDS(vt_boot, 'fitted-models/vt_boot-diff_2022.RDS')
}else{
  vt_boot <- readRDS('fitted-models/vt_boot-diff_2022.RDS')
}
```
